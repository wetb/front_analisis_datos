...
const thirdQuestionData = fetchData('jornada_matricula/')
...
const ThirdQuestion = () => {
    ...
    let filtersArray = data.filter(
        (obj, index, self) =>
            index ===
            self.findIndex((t) => t.TIPO_JORNADA === obj.TIPO_JORNADA)
    );

    console.log(filtersArray)

    const fixedData = data.reduce((acc, cur) => {
        const { ANNO_INF, TIPO_JORNADA, TOTAL_MATRICULA } = cur;
        if (!acc[ANNO_INF]) acc[ANNO_INF] = {};
        if (!acc[ANNO_INF][TIPO_JORNADA]) acc[ANNO_INF][TIPO_JORNADA] = 0;
        acc[ANNO_INF][TIPO_JORNADA] += TOTAL_MATRICULA;
        return acc;
    }, {});
    ...
    const isEducationLevelSelected = (educationLevel) => (selectedLevels.includes(educationLevel.TIPO_JORNADA) || selectedLevels.length === 0);
    ...
    return (
        <>
            ...
            <h2 className='flex items-center pt-10 pb-4 text-2xl font-semibold opacity-60 gap-x-2'>
                ...
                Total de matriculas por año por tipo de jornada academica
            </h2>
                    ...
                    <div className="sm:mt-6 hidden sm:flex sm:start sm:space-x-2">
                        ...
                        >
                            {filtersArray.map((item) => (
                                <MultiSelectBoxItem
                                    key={item.id}
                                    value={item.TIPO_JORNADA}
                                    text={item.TIPO_JORNADA}
                                />
                            ))}
                        </MultiSelectBox>
                    </div>
                    <Table className="max-h-96 overscroll-none mt-6">
                        <TableHead>
                            <TableRow>
                                ...
                                <TableHeaderCell className='bg-white'>TIPO JORNADA</TableHeaderCell>
                                <TableHeaderCell className="bg-white text-right">TOTAL_MATRICULAS</TableHeaderCell>
                            </TableRow>
                        </TableHead>
                            ...
                            {data
                                ...
                                .map((item) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.TIPO_JORNADA}</TableCell>
                                        <TableCell className="text-right">{item.TOTAL_MATRICULA}</TableCell>
                                    </TableRow>
                                ))}
                        </TableBody>
                    </Table>
