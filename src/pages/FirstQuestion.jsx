...
const firstQuestionData = fetchData('matriculas_municipio/')

const FirstQuestion = () => {
    const data = firstQuestionData.read()
    const top5 = data.sort((a, b) => b - a).slice(0, 10)
    ...
    return (
        <>
            ...
            <h2 className='flex items-center pt-10 pb-4 text-2xl font-semibold opacity-60 gap-x-2'>
                ...
                TOP 10 MUNICIPIOS CON MAYOR CANTIDAD DE MATRICULAS DURANTE (2018-2019)
            </h2>
            <Card>
                <Table className="mt-1">
                    <TableHead>
                        <TableRow>
                            <TableHeaderCell>MUNICIPO</TableHeaderCell>
                            <TableHeaderCell>TOTAL MATRICULAS</TableHeaderCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {top5.map((item) => (
