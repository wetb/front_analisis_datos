...
const secondQuestionData = await fetchData('genero_matricula/')
...
const SecondQuestion = () => {
    ...
    const valueFormatter = (number) => {
        console.log(total);
        console.log(number);
        `${Intl.NumberFormat("us").format(127221).toString()}(${(calculatePercentage(127221, total)).toFixed(1)}%)`;
    }
    return (
        <>
            <div
            ...
            <h2 className='flex items-center pt-10 pb-4 text-2xl font-semibold opacity-60 gap-x-2'>
                ...
                ¿Cuál es la cantidad de hombres y mujeres que se matricularon en colegios durante los años 2018 a 2019?
            </h2>
            <div className='w-full'>
                <Suspense fallback={<div>Loading...</div>}>
                    <Card className="max-w-lg">
                        <Title className='text-center'>
                            por genero matriculod@s
                        </Title>
                        <DonutChart
                            className="mt-6"
                            data={data}
                            category="TOTAL_MATRICULA"
                            index="GENERO"
